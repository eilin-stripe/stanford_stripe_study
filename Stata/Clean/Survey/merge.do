...
** this do-file merges waves 1, 2, and 3 data
...
import delimited "`raw_dir'sie_w1.csv", varnames(1) encoding(ISO-8859-1) clear
drop if inlist(_n, 1, 2, 3, 4, 5, 6)
qui compress
tempfile label1
save "`label1'", replace

drop _all
import delimited "/Users/eilin/Documents/SIE/01_raw_data/Stripe+Enterprise+Survey_wave2_July+15,+2019_12.csv", encoding(ISO-8859-1) clear
drop if status =="Survey Preview"
drop if externalreference == "test"
drop if externalreference == "" & responseid!="R_1PT2lyQnCUUrRUu"
qui compress

append using "`label1'", generate(SurveyRound) force
...
* drop tests and some more * \\ef
drop if ExternalReference=="acct_1Cx7icJMm4cb9RyC" & FinishedTemp == "False"
...
replace KeyBestAvenue = "1" if KeyBestAvenue == "Best avenue for my ideas/goods/services"
...
destring KeyOther, replace force		//ef added force
...
destring SourcesOther, force replace			//ef added force
...
// drop duplicate IDs (likely from resending survey in june to incomplete round 1 participants i)
drop if merchant_id=="acct_14g1vmBI3TuE3eYh"
 
// clean up duplicate merchant id
bysort merchant_id: gen n = _n
drop if n == 2 & (FinishedTemp =="FALSE" | FinishedTemp =="False")
drop if (FinishedTemp=="False" | FinishedTemp=="FALSE") & (merchant_id == "acct_19w2FDAgVTyTAZP7" | merchant_id == "acct_17r1JnFmaDiomWIT" | merchant_id=="acct_1A5f27CPBRi7Kg3Z" | merchant_id=="acct_1AEnybA3WRtyG0cI" | merchant_id=="acct_1B3PltywazPTCZONHOcR" | merchant_id=="acct_1Cx7icJMm4cb9RyC")
drop if merchant_id=="acct_16hSGgBcPB3O5mcM" & FinishedTemp == "False"
drop if merchant_id=="acct_1AxEr8DOZUCvjAiM" & FinishedTemp == "False"
drop n
...
** small
import excel "`raw_dir'sample_us_190402_1.xlsx", sheet("small") firstrow clear
rename _id merchant_id
*merge 1:1 merchant_id using "/Users/eilin/Documents/SIE/sta_files/round1.dta"
merge 1:1 merchant_id using "`clean_dir'round1.dta"
gen strata = "small" if _merge == 3
drop _merge
save "`clean_dir'round1.dta", replace

** merge in big
import excel "/Users/eilin/Documents/SIE/01_raw_data/sample_us_190402_1.xlsx", sheet("big") firstrow clear
rename _id merchant_id
merge 1:1 merchant_id using "`clean_dir'round1.dta"
replace strata = "big" if _merge == 3
drop _merge
save "`clean_dir'round1.dta", replace

** funded
import excel "/Users/eilin/Documents/SIE/01_raw_data/sample_us_190402_1.xlsx", sheet("funded") firstrow clear
rename _id merchant_id
merge 1:1 merchant_id using "`clean_dir'round1.dta"
replace strata = "funded" if _merge == 3
drop _merge
drop if Status == ""
save "`clean_dir'round1.dta", replace
