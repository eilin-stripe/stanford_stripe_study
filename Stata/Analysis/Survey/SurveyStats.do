...
estpost tabulate Progress
esttab using "`output'/Progress.tex" , ///
...
foreach var of local tab_vars {
    estpost tabulate `var'
    esttab using "`output'/`var'.tex" , ///
        cells("b(label(freq)) pct(fmt(2)) cumpct(fmt(2))") ///
        varlabels(, blist(Total "\hline "))      ///
        nonumber nomtitle noobs replace
}
...
foreach var of local log_hist_vars {
    pretty (hist `var', xlogbase(1.4) frac zeros(1)), save("`output'/`var'.eps") ///
        name("`var'") xtitle("")
}
...
pretty (scatter Education EarningsPast12Months, xlogbase(1.2) ///
    ...
    save("`output'/EarningsVsEducation.eps")
...
pretty (scatter Education MeanEarningsByEduc if EducationTag == 1, xlogbase(1.2) ///
    ...
    save("`output'/MeanEarningsVsEducation.eps")
...
graph export "`output'/BoxEducationVsEarnings.eps", replace
...
graph export "`output'/PredEarningsVsPredEmp.eps", replace
...
pretty (hist NewRatio, xlogbase(1.2) frac ) ,  name("Ratio12OverThreeTimesFour") xtitle("") save("`output'/Ratio12to3.eps")
pretty (hist NewRatio2, xlogbase(1.2) frac ) ,  name("Ratio9OverThreeTimesThree") xtitle("") save("`output'/Ratio9to3.eps")
pretty (hist Ratio12, xlogbase(1.4) frac ) ,  name("Ratio12") xtitle("") save("`output'/Ratio12.eps")
pretty (hist Ratio3, xlogbase(1.4) frac ) ,  name("Ratio3") xtitle("") save("`output'/Ratio3.eps")

pretty (hist Ratio12_1, xlogbase(1.2) frac) ,  name("Ratio12_1") xtitle("") save("`output'/Ratio12_1.eps")
pretty (hist Ratio3_1, xlogbase(1.2) frac) ,  name("Ratio3_1") xtitle("") save("`output'/Ratio3_1.eps")
