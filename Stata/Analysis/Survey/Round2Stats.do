...
local output_stats = "`output'/Round2Stats"

** Load in the Stripe Panel data
local survey = "`clean_main_survey'/Round2.dta"
*******************************************************************************
** Stats
*******************************************************************************
use "`survey'", clear
...
gen DurationInMin = Duration / 60
pretty ( hist DurationInMin , xlogbase(1.2)), xtitle("Duration in Minutes")

local horiz_vars = "PhysicalOnlineBoth NumKPIs HowFrequentKPIs " ///
    + "ProblemAdressingMethod TargetsTimeFrame TargetsDifficulty " ///
    + "HowPromoteEmployees UnderperformingEmployee RecordsMethod " ///
    + "RecordsUsedHow"
...
local log_hist_vars = "EarningsPast12Months Good3Months " ///
    + "Bad3Months Good12Months Bad12Months RevPast12Months " ///
        + "Predict3Months Predict12Months Duration" ///
...
local percent_vars = "PercRevOnline PercRevStripe"
foreach var of local percent_vars {
    replace `var' = `var' * 100 if `var' > 0 & `var' < 1
    replace `var' = round(`var') if `var' > 0 & `var' < 1
    evenbin `var' , dif(10) maxcutoff(100) zero
